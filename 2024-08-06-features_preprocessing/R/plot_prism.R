#' Plot PRISM climate Data on a Spatial Grid
plot_prism <- function(grid, plot_data, column, col_palette = "chelix") {
  ...
  if(col_palette == "chelix"){
    ...
    col_values <- seq(0, 1, 0.1)
  }else{
    color_palette <- (pals::ocean.thermal(50))
    col_values <- seq(0, 1, 0.1)
  }
  ...
  ret_plot <- ggplot() +
    ...
    geom_polygon(data = sp_state_borders,
                 aes(long, lat, group = group), fill = "transparent", col="gray30", linewidth = 0.75) +
