...
library(ggspatial)
library(ggplot2)
library(RColorBrewer) # ** used to create colorblind-friendly color palette for map
...
conf_pt <- st_transform(conf_pt, st_crs(world_flagged)) # ** change world_robin to world_flagged

# ** insert creation of ocean polygon and matching projection
# ocean
ocean <- st_polygon(list(cbind(c(seq(-180, 179, len = 100), rep(180, 100), 
                                 seq(179, -180, len = 100), rep(-180, 100)),
                               c(rep(-80, 100), seq(-79, 89, len = 100),
                                 rep(90, 100), seq(89, -80, len = 100))))) %>%
  st_sfc(crs = "WGS84") %>%
  st_as_sf()
ocean <- st_transform(ocean, st_crs(world_flagged))
